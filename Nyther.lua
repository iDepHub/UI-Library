...
local function awaitKeyInput()
    mainFrame.Visible = false

    local _ksBlur = Instance.new("BlurEffect")
    _ksBlur.Size   = 56
    _ksBlur.Parent = game:GetService("Lighting")

    local ksSG = Instance.new("ScreenGui")
    ksSG.Name           = "iDepKeySystem"
    ksSG.ResetOnSpawn   = false
    ksSG.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
    ksSG.DisplayOrder   = 1100
    ksSG.Parent         = playerGui
    ...
    local card = Instance.new("Frame")
    card.Size             = UDim2.new(0, 400, 0, 268)
    card.AnchorPoint      = Vector2.new(0.5, 0.5)
    card.Position         = UDim2.new(0.5, 0, 1.6, 0)   
    card.BackgroundColor3 = Color3.fromRGB(4, 4, 4)
    card.BorderSizePixel  = 0
    card.ZIndex           = 2
    card.Parent           = ksSG
    ...
    cardStroke.Color       = T.Accent
    cardStroke.Thickness   = 1.5
    cardStroke.Transparency = 0.25
    cardStroke.Parent      = card
    table.insert(_accentObjs, {cardStroke, "Color"})

    local ksTopBar = Instance.new("Frame")
    ksTopBar.Size             = UDim2.new(1, 0, 0, 36)
    ksTopBar.BackgroundColor3 = Color3.fromRGB(3, 3, 3)
    ksTopBar.BorderSizePixel  = 0
    ksTopBar.ZIndex           = 3
    ksTopBar.Parent           = card
    Instance.new("UICorner", ksTopBar).CornerRadius = UDim.new(0, 6)
    local ksTopFill = Instance.new("Frame")
    ksTopFill.Size             = UDim2.new(1, 0, 0.55, 0)
    ksTopFill.Position         = UDim2.new(0, 0, 0.45, 0)
    ksTopFill.BackgroundColor3 = Color3.fromRGB(3, 3, 3)
    ksTopFill.BorderSizePixel  = 0
    ksTopFill.ZIndex           = 3
    ksTopFill.Parent           = ksTopBar

    local ksTopLine = Instance.new("Frame")
    ksTopLine.Size             = UDim2.new(1, 0, 0, 1)
    ksTopLine.Position         = UDim2.new(0, 0, 1, -1)
    ksTopLine.BackgroundColor3 = T.Accent
    ksTopLine.BorderSizePixel  = 0
    ksTopLine.ZIndex           = 4
    ksTopLine.Parent           = ksTopBar
    table.insert(_accentObjs, {ksTopLine, "BackgroundColor3"})

    local ksDot = Instance.new("Frame")
    ksDot.Size             = UDim2.new(0, 8, 0, 8)
    ksDot.Position         = UDim2.new(0, 12, 0.5, -4)
    ksDot.BackgroundColor3 = T.Accent
    ksDot.BorderSizePixel  = 0
    ksDot.ZIndex           = 5
    ksDot.Parent           = ksTopBar
    Instance.new("UICorner", ksDot).CornerRadius = UDim.new(1, 0)
    table.insert(_accentObjs, {ksDot, "BackgroundColor3"})

    local ksTitleLbl = Instance.new("TextLabel")
    ksTitleLbl.Size                   = UDim2.new(1, -28, 1, 0)
    ksTitleLbl.Position               = UDim2.new(0, 28, 0, 0)
    ksTitleLbl.BackgroundTransparency = 1
    ksTitleLbl.Text                   = "iDepHub  ·  Key System"
    ksTitleLbl.TextColor3             = T.Text
    ksTitleLbl.TextSize               = 13
    ksTitleLbl.Font                   = Enum.Font.GothamSemibold
    ksTitleLbl.TextXAlignment         = Enum.TextXAlignment.Left
    ksTitleLbl.ZIndex                 = 5
    ksTitleLbl.Parent                 = ksTopBar

    local ksBody = Instance.new("Frame")
    ksBody.Size                   = UDim2.new(1, 0, 1, -36)
    ksBody.Position               = UDim2.new(0, 0, 0, 36)
    ksBody.BackgroundTransparency = 1
    ksBody.BorderSizePixel        = 0
    ksBody.ZIndex                 = 3
    ksBody.Parent                 = card

    local ksSubLbl = Instance.new("TextLabel")
    ksSubLbl.Size                   = UDim2.new(1, -30, 0, 20)
    ksSubLbl.Position               = UDim2.new(0, 15, 0, 18)
    ksSubLbl.BackgroundTransparency = 1
    ksSubLbl.Text                   = "Ingresa tu key de acceso para continuar"
    ksSubLbl.TextColor3             = T.TextDim
    ksSubLbl.TextSize               = 12
    ksSubLbl.Font                   = Enum.Font.Gotham
    ksSubLbl.TextXAlignment         = Enum.TextXAlignment.Left
    ksSubLbl.ZIndex                 = 4
    ksSubLbl.Parent                 = ksBody

    local ksSep = Instance.new("Frame")
    ksSep.Size             = UDim2.new(1, -30, 0, 1)
    ksSep.Position         = UDim2.new(0, 15, 0, 46)
    ksSep.BackgroundColor3 = T.BorderDim
    ksSep.BorderSizePixel  = 0
    ksSep.ZIndex           = 4
    ksSep.Parent           = ksBody

    local ksInputBg = Instance.new("Frame")
    ksInputBg.Size             = UDim2.new(1, -30, 0, 42)
    ksInputBg.Position         = UDim2.new(0, 15, 0, 58)
    ksInputBg.BackgroundColor3 = T.Elem
    ksInputBg.BorderSizePixel  = 0
    ksInputBg.ZIndex           = 4
    ksInputBg.Parent           = ksBody
    Instance.new("UICorner", ksInputBg).CornerRadius = UDim.new(0, 4)

    local ksInputStroke = Instance.new("UIStroke")
    ksInputStroke.Color     = T.BorderDim
    ksInputStroke.Thickness = 1
    ksInputStroke.Parent    = ksInputBg

    local ksTextBox = Instance.new("TextBox")
    ksTextBox.Size                = UDim2.new(1, -16, 1, 0)
    ksTextBox.Position            = UDim2.new(0, 8, 0, 0)
    ksTextBox.BackgroundTransparency = 1
    ksTextBox.Text                = ""
    ksTextBox.PlaceholderText     = "DH-XXXX-XXXX"
    ...
    ksStatusLbl.Size                   = UDim2.new(1, -30, 0, 20)
    ksStatusLbl.Position               = UDim2.new(0, 15, 0, 108)
    ksStatusLbl.BackgroundTransparency = 1
    ksStatusLbl.Text                   = ""
    ksStatusLbl.TextColor3             = Color3.fromRGB(220, 60, 60)
    ksStatusLbl.TextSize               = 12
    ksStatusLbl.Font                   = Enum.Font.Gotham
    ksStatusLbl.TextXAlignment         = Enum.TextXAlignment.Left
    ksStatusLbl.ZIndex                 = 4
    ksStatusLbl.Parent                 = ksBody

    local ksBtnFrame = Instance.new("Frame")
    ksBtnFrame.Size             = UDim2.new(1, -30, 0, 42)
    ksBtnFrame.Position         = UDim2.new(0, 15, 0, 136)
    ksBtnFrame.BackgroundColor3 = T.Elem
    ksBtnFrame.BorderSizePixel  = 0
    ksBtnFrame.ZIndex           = 4
    ksBtnFrame.Parent           = ksBody
    Instance.new("UICorner", ksBtnFrame).CornerRadius = UDim.new(0, 4)
    local ksBtnStroke = Instance.new("UIStroke")
    ksBtnStroke.Color     = T.BorderDim
    ksBtnStroke.Thickness = 1
    ksBtnStroke.Parent    = ksBtnFrame

    local ksBtnAccent = Instance.new("Frame")
    ksBtnAccent.Size             = UDim2.new(0, 3, 0.6, 0)
    ksBtnAccent.Position         = UDim2.new(0, 8, 0.2, 0)
    ksBtnAccent.BackgroundColor3 = T.Accent
    ksBtnAccent.BorderSizePixel  = 0
    ksBtnAccent.ZIndex           = 5
    ksBtnAccent.Parent           = ksBtnFrame
    Instance.new("UICorner", ksBtnAccent).CornerRadius = UDim.new(1, 0)
    table.insert(_accentObjs, {ksBtnAccent, "BackgroundColor3"})

    local ksSubmitBtn = Instance.new("TextButton")
    ksSubmitBtn.Size                   = UDim2.new(1, 0, 1, 0)
    ksSubmitBtn.BackgroundTransparency = 1
    ksSubmitBtn.Text                   = "VERIFICAR KEY"
    ksSubmitBtn.TextColor3             = T.Text
    ksSubmitBtn.TextSize               = 13
    ksSubmitBtn.Font                   = Enum.Font.GothamBold
    ksSubmitBtn.ZIndex                 = 6
    ksSubmitBtn.Parent                 = ksBtnFrame

    local ksFooter = Instance.new("Frame")
    ksFooter.Size             = UDim2.new(1, 0, 0, 20)
    ksFooter.Position         = UDim2.new(0, 0, 0, 210)
    ksFooter.BackgroundColor3 = Color3.fromRGB(3, 3, 3)
    ksFooter.BorderSizePixel  = 0
    ksFooter.ZIndex           = 3
    ksFooter.Parent           = ksBody

    local ksFooterLine = Instance.new("Frame")
    ksFooterLine.Size             = UDim2.new(1, 0, 0, 1)
    ksFooterLine.BackgroundColor3 = T.BorderDim
    ksFooterLine.BorderSizePixel  = 0
    ksFooterLine.ZIndex           = 4
    ksFooterLine.Parent           = ksFooter

    local ksDiscordLbl = Instance.new("TextLabel")
    ksDiscordLbl.Size                   = UDim2.new(1, 0, 1, 0)
    ksDiscordLbl.BackgroundTransparency = 1
    ksDiscordLbl.Text                   = "discord.gg/FxnmyRxkzv"
    ksDiscordLbl.TextColor3             = T.TextDim
    ksDiscordLbl.TextSize               = 10
    ksDiscordLbl.Font                   = Enum.Font.GothamSemibold
    ksDiscordLbl.TextXAlignment         = Enum.TextXAlignment.Center
    ksDiscordLbl.ZIndex                 = 4
    ksDiscordLbl.Parent                 = ksFooter
    ...
    local _submitActive = true

    local function _shakeCard()
    ...
    local function setStatus(msg, success)
        if success then
            ksStatusLbl.TextColor3 = T.Accent
            ksStatusLbl.Text       = msg
        else
            ksStatusLbl.TextColor3 = Color3.fromRGB(220, 60, 60)
            ksStatusLbl.Text       = msg
            TweenService:Create(ksInputStroke, TweenInfo.new(0.1), {
                Color = Color3.fromRGB(180, 40, 40)
            }):Play()
            TweenService:Create(ksInputStroke, TweenInfo.new(1.2), {
                Color = T.BorderDim
            }):Play()
            _shakeCard()
        end
    ...
    local function closeKeyUI()
        ...
        TweenService:Create(_ksBlur, TweenInfo.new(0.5), { Size = 0 }):Play()
    ...
    ksSubmitBtn.MouseEnter:Connect(function()
        if not _submitActive then return end
        TweenService:Create(ksBtnFrame,  TweenInfo.new(0.1), {BackgroundColor3 = T.ElemHov}):Play()
        TweenService:Create(ksBtnStroke, TweenInfo.new(0.1), {Color = Color3.fromRGB(48, 48, 48)}):Play()
    end)
    ksSubmitBtn.MouseLeave:Connect(function()
        TweenService:Create(ksBtnFrame,  TweenInfo.new(0.1), {BackgroundColor3 = T.Elem}):Play()
        TweenService:Create(ksBtnStroke, TweenInfo.new(0.1), {Color = T.BorderDim}):Play()
    end)
    local _onSubmit = nil 

    local function _trySubmit()
        if not _submitActive then return end
        local key = ksTextBox.Text:upper():gsub("%s+", "")
        if key == "" then
            setStatus("❌  Ingresa una key", false)
            return
        end
        if not key:match("^DH%-%a%a%a%a%-%a%a%a%a$") then
            setStatus("❌  Formato inválido  ·  DH-XXXX-XXXX", false)
            return
        end
        ksSubmitBtn.Text = "Verificando..."
        _submitActive    = false
        ksStatusLbl.Text = ""
        if _onSubmit then _onSubmit(key) end
    end

    ksSubmitBtn.MouseButton1Click:Connect(_trySubmit)
    ksTextBox.FocusLost:Connect(function(enter)
        if enter then _trySubmit() end
    end)

    local function onSubmit(callback)
        _onSubmit = callback
    end

    return onSubmit, setStatus, closeKeyUI
end
