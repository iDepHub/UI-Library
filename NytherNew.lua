...
local function NewToggle(parent, label, sub, default, callback, iconName)
    ...
    track.BackgroundColor3 = default and Theme.Accent or Theme.Toggle
    ...
    knob.Parent           = track
    Corner(knob, 8)
    local knobStroke = Stroke(knob, Theme.Accent, 1)
    ...
    local function setState(newState)
        ...
        TweenService:Create(knob, TweenInfo.new(0.15, Enum.EasingStyle.Sine, Enum.EasingDirection.InOut), {
    ...
    btn.MouseEnter:Connect(function()
        if locked then return end
        TweenService:Create(f, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Hover}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.1), {Color = Theme.Accent}):Play()
    end)
    btn.MouseLeave:Connect(function()
        if locked then return end
        TweenService:Create(f, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
        TweenService:Create(stroke, TweenInfo.new(0.1), {Color = Theme.Line}):Play()
    end)
...
local function NewButton(parent, label, sub, callback, iconName)
    ...
    btn.MouseButton1Click:Connect(function()
        TweenService:Create(f, TweenInfo.new(0.05), {BackgroundColor3 = Theme.Accent}):Play()
...
local function NewInput(parent, label, placeholder, callback)
    ...
    textBox.Focused:Connect(function()
        TweenService:Create(boxStroke, TweenInfo.new(0.1), {Color = Theme.Accent}):Play()
        TweenService:Create(f,         TweenInfo.new(0.1), {BackgroundColor3 = Theme.Hover}):Play()
        TweenService:Create(stroke,    TweenInfo.new(0.1), {Color = Theme.Accent}):Play()
    end)
    textBox.FocusLost:Connect(function(enterPressed)
        TweenService:Create(boxStroke, TweenInfo.new(0.1), {Color = Theme.Line}):Play()
        TweenService:Create(f,         TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
        TweenService:Create(stroke,    TweenInfo.new(0.1), {Color = Theme.Line}):Play()
...
local function NewKeybind(parent, label, sub, defaultKey, callback, iconName)
    ...
    keyBtn.MouseButton1Click:Connect(function()
        ...
        keyLbl.Text = "..."
        keyLbl.TextColor3 = Theme.Text
        TweenService:Create(keyStroke, TweenInfo.new(0.1), {Color = Theme.Accent}):Play()
        TweenService:Create(keyBg, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Accent}):Play()
        ...
        conn = UserInputService.InputBegan:Connect(function(inp, gp)
            ...
            if inp.UserInputType == Enum.UserInputType.Keyboard then
                ...
                keyLbl.TextColor3 = Theme.Text
                TweenService:Create(keyStroke, TweenInfo.new(0.1), {Color = Theme.Line}):Play()
                TweenService:Create(keyBg, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Toggle}):Play()
                listening = false
                conn:Disconnect()
            end
        end)
...
local function NewColorPicker(parent, label, sub, defaultColor, callback, iconName)
    ...
    header.MouseEnter:Connect(function()
        ...
        TweenService:Create(hStroke, TweenInfo.new(0.1), {Color = Theme.Accent}):Play()
    end)

    header.MouseLeave:Connect(function()
        if not expanded then
            TweenService:Create(header, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
        end
        TweenService:Create(hStroke, TweenInfo.new(0.1), {Color = Theme.Line}):Play()
    end)
...
local function NewSearchPanel(searchTabData, opts)
    ...
    sendBtn.MouseEnter:Connect(function()
        TweenService:Create(sendBtn, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Accent}):Play()
    end)
    sendBtn.MouseLeave:Connect(function()
        TweenService:Create(sendBtn, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Hover}):Play()
    end)

    local function SetSelected(name, rowData)
        for _, rf in ipairs(weaponRowFrames) do
            TweenService:Create(rf.frame, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
            rf.lbl.TextColor3 = Theme.Dim
        end
        selectedWeapon = name
        if name and rowData then
            TweenService:Create(rowData.frame, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Hover}):Play()
        ...
        for _, weaponName in ipairs(unique) do
            if filterLower == "" or weaponName:lower():find(filterLower, 1, true) then
                ...
                rowBtn.MouseLeave:Connect(function()
                    if selectedWeapon == weaponName then return end
                    TweenService:Create(row, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
                end)
            end
        end
    end
...
UserInputService.InputChanged:Connect(function(inp)
    if isDragging and (inp.UserInputType == Enum.UserInputType.MouseMovement or inp.UserInputType == Enum.UserInputType.Touch) then
        ...
        local newX = math.clamp(frameStart.X.Offset + delta.X, 0, math.max(0, viewport.X - winSize.X))
        local newY = math.clamp(frameStart.Y.Offset + delta.Y, 0, math.max(0, viewport.Y - winSize.Y))
...
local function NewBodyPartSelector(parent, label, sub, selectedParts, allParts, defaultParts, extRefreshTable, iconName)
    ...
    local function makePartBtn(btnLabel, partName, x, y, w, h, mirrorPart)
        ...
        btn.MouseEnter:Connect(function()
            if not selectedParts[partName] then
                TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3 = Theme.Hover}):Play()
            end
        end)
        btn.MouseLeave:Connect(function()
            if not selectedParts[partName] then
                TweenService:Create(btn, TweenInfo.new(0.08), {BackgroundColor3 = Theme.Raised}):Play()
            end
        end)
    end
    ...
    local function makeActionBtn(btnLabel, x, callback)
        ...
        ab.MouseEnter:Connect(function()
            TweenService:Create(ab, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Hover}):Play()
        end)
        ab.MouseLeave:Connect(function()
            TweenService:Create(ab, TweenInfo.new(0.1), {BackgroundColor3 = Theme.Raised}):Play()
        end)
