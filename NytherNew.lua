...
local function setAccentColor(color)
    ...
    for _, fn in ipairs(_accentCallbacks) do pcall(fn, color) end
    for _, data in pairs(tabs) do
        if data.isActive then
            pcall(function() data.button.TextColor3 = color end)
            pcall(function() data.icon.ImageColor3  = color end)
        end
    end
end
...
local function CenterWindow()
    ...
    Main.Position  = UDim2.new(0, (viewport.X - WINDOW_SIZE.X.Offset) / 2,
                                0, (viewport.Y - WINDOW_SIZE.Y.Offset) / 2)
end
...
CloseBtn.MouseLeave:Connect(function()
    SafeTween(CloseBtn, TweenInfo.new(0.15), { TextColor3 = Theme.Dim })
end)
...
task.spawn(function()
    while Main and Main.Parent and PillDot and PillDot.Parent do
        ...
        task.wait(0.7)
        if not (Main and Main.Parent and PillDot and PillDot.Parent) then break end
...
UserInputService.InputChanged:Connect(function(input)
    if dragging and input.UserInputType == Enum.UserInputType.MouseMovement then
        if not (Main and Main.Parent) then dragging = false; return end
        ...
        Main.Position = UDim2.new(0,
            math.clamp(startPos.X.Offset + delta.X, 0, math.max(0, vpSize.X - winSize.X)),
            0,
            math.clamp(startPos.Y.Offset + delta.Y, 0, math.max(0, vpSize.Y - winSize.Y))
        )
    end
end)
...
task.spawn(function()
    while Main and Main.Parent and DividerScan and DividerScan.Parent do
        ...
        task.wait(2.6)
        if not (Main and Main.Parent and DividerScan and DividerScan.Parent) then break end
...
local function CreateSlider(parent, labelText, minVal, maxVal, defaultVal, callback)
    ...
    return {
        ...
        setValue = function(v)
            setPercent(math.clamp((v - minVal) / (maxVal - minVal), 0, 1))
        end,
    }
end
...
local function CreateKeyBind(parent, labelText, defaultKey, callback)
    ...
    connection = UserInputService.InputBegan:Connect(function(input, gameProcessed)
        if gameProcessed or not isCapturing then return end
        ...
        if key == Enum.KeyCode.LeftShift   or key == Enum.KeyCode.RightShift  or
           ...
           key == Enum.KeyCode.CapsLock    or key == Enum.KeyCode.LeftMeta     or
           key == Enum.KeyCode.RightMeta then return end
        if key ~= Enum.KeyCode.Unknown then
            setKey(tostring(key):gsub("Enum.KeyCode.", ""))
            stopCapture()
        end
    end)
...
local function NewColorPicker(parent, labelText, subText, defaultColor, callback, updateCanvas)
    ...
    sqBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            dragSq = true; updateFromSquare(inp.Position.X, inp.Position.Y) end
    end)
    hueBtn.InputBegan:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            dragHue = true; updateFromHue(inp.Position.Y) end
    end)
    ...
    table.insert(activeConnections, UserInputService.InputEnded:Connect(function(inp)
        if inp.UserInputType == Enum.UserInputType.MouseButton1 then
            dragSq = false; dragHue = false end
    end))
...
local function CreateTab(tabName, iconName)
    ...
    return {
        ...
        setBuilder = function(fn)
            assert(type(fn) == "function", "setBuilder expects a function, got " .. type(fn))
            data.buildFn = fn
        end,
        activate = function()
            ActivateTab(tabName)
...
local function SetTitle(text)   Title.Text    = text end
local function SetVersion(text) Version.Text  = text end
local function SetPillText(text) PillText.Text = text end
local function Open()    Main.Visible = true  end
local function Close()   Main.Visible = false end
